# AI Agent Context — disclosur.dev

## Summary

*one paragraph: what this site is, what it does*

AgeSDK is a React Native SDK that handles AB 1043 age signal compliance end-to-end. It requests the age bracket signal from iOS and Android OS APIs, applies the correct COPPA and CCPA privacy rules per bracket, logs a timestamped compliance event, and surfaces coverage data in a developer dashboard. App developers drop in one npm package and are compliant before the January 1, 2027 enforcement deadline.
...
A React Native developer or small-team CTO at a consumer app with 10K–500K MAU. Has California users. Received a legal email about AB 1043 or saw it on Twitter. Has never dealt with age compliance. Will self-serve, will not talk to sales, and will pay $79–249/mo if the README solves the problem in under 30 minutes.
...
`proj=disclosur.dev`, so these all work either from this dir or from `sites/`:

| Command | What it does |
|---|---|
| `make buildsh` *(from `sites/`)* | Drop into a bash shell inside the docker container at `/usr/src/app` (= `sites/` mounted in). |
| `make run` *(from here)* / `make run proj=disclosur.dev` *(from `sites/`)* | `pnpm install` then start dev server (auto-detected). |
| `make check-vite proj=disclosur.dev` | Start the dev server, skipping install. |
| `make test proj=disclosur.dev` | `pnpm install` + `pnpm build` + `pnpm test`. **Hard-fails outside docker** — `make buildsh` first, or `docker exec`. |
...
docker exec -w /usr/src/app <name> make test proj=disclosur.dev
```
...
- **Deploy:** `wrangler deploy` (locally) or via Cloudflare's Git integration on push.
  ...
  `cd ../portfolio && make run ARGS="deploy disclosur.dev"` runs `gh repo create` and
...
- **Live URL:** https://disclosur.dev/  *(update once first deploy succeeds)*
...
Landing page targeting AB 1043 compliance SDK. Docs site with per-framework guides (Expo, bare RN). Blog: long-form law explainers for non-technical founders, technical tutorials for developers, comparison posts (AB 1043 vs COPPA, build vs buy). Format mix: one pillar guide, weekly tutorial posts, decision-tree tools, and a plain-language FAQ optimized for zero-KD law-specific queries.

### Post-deploy checklist (do these once after the first successful deploy)

- [ ] Verify in **Google Search Console** at https://search.google.com/search-console — add as `sc-domain:disclosur.dev` property; verify via DNS TXT record. Until this is done, no SEO traffic data is observable for this site (and the workspace-wide `30 commercial sites with traffic` goal can't credit it).
- [ ] Submit the sitemap (`https://disclosur.dev/sitemap.xml`) inside GSC.
- [ ] Update the **Live URL** above with the actual deploy URL.
- [ ] Run `make run ARGS="cleanup"` from `sites/portfolio/` so `data/portfolio.json` reflects the new project's state (and `project status disclosur.dev` resolves cleanly).
...
`portfolio project check disclosur.dev` (run from `sites/portfolio/`).
